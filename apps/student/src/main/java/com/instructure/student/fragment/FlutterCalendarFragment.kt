...
class FlutterCalendarFragment : FlutterFragment() {
    ...
    override fun onHiddenChanged(hidden: Boolean) {
        // Workaround for an issue where the engine does no re-attach additional FlutterCalendarFragments have been added and removed
...
class CalendarScreenChannel{
    ...
    var onTodaySelected: ((isTodaySelected: Boolean) -> Unit)? = null
    ...
    init {
        channel.setMethodCallHandler { call, _ ->
            when(call.method) {
                "isTodaySelected" -> onTodaySelected?.invoke(call.arguments as Boolean)
                "routeToItem" -> {
    ...
    fun goToToday() = channel.invokeMethod("todayClicked", null)
}
